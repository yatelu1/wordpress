...
function AjaxSendComment()
{
   var gi = getXMLInstant(1), r = $('comment_reply_ID').value, t, c, now = parseInt(Math.random()*1000);
	...
	if(author!= "admin"){
if(!jQuery.cookie('cksky_author')){
	jQuery.cookie('cksky_author',author, { path: '/', expires: 360 });
	jQuery.cookie('cksky_email',email, { path: '/', expires: 360 });
	jQuery.cookie('cksky_url',url, { path: '/', expires: 360 });

	}
	else if( jQuery.cookie('cksky_author')!=author || jQuery.cookie('cksky_email')!=email || jQuery.cookie('cksky_email')!=email){
	jQuery.cookie('cksky_author',author, { path: '/', expires: 360 });
	jQuery.cookie('cksky_email',email, { path: '/', expires: 360 });
	jQuery.cookie('cksky_url',url, { path: '/', expires: 360 });
	}
